...
class RateLimiter {
  ...
  async checkVideoLimit(ip) {
    if (!this.KV) {
      console.warn("⚠️ FLUX_KV 未綁定，跳過影片限制");
      return { allowed: true };
    }
    const key = `video_limit:${ip}`;
    const hourWindow = 3600 * 1000; // 1小時 (毫秒)
    const dayWindow = 86400 * 1000; // 1天 (毫秒)
    const maxHourly = CONFIG.VIDEO.rateLimit.maxRequestsPerHour;
    const maxDaily = CONFIG.VIDEO.rateLimit.maxRequestsPerDay;
    
    try {
      const rawData = await this.KV.get(key);
      let data = rawData ? JSON.parse(rawData) : { hourly: [], daily: [] };
      const now = Date.now();
      
      // 清理過期的時間戳
      data.hourly = data.hourly.filter(ts => now - ts < hourWindow);
      data.daily = data.daily.filter(ts => now - ts < dayWindow);
      
      // 檢查每小時限制
      if (data.hourly.length >= maxHourly) {
        const oldest = data.hourly[0];
        const resetTime = oldest + hourWindow;
        const waitMin = Math.ceil((resetTime - now) / 60000);
        return {
          allowed: false,
          reason: `🎬 影片生成限額已滿 (${maxHourly}個/小時)。請休息 ${waitMin} 分鐘後再來。`,
          remaining: 0
        };
      }
      
      // 檢查每天限制
      if (data.daily.length >= maxDaily) {
        const oldest = data.daily[0];
        const resetTime = oldest + dayWindow;
        const waitHours = Math.ceil((resetTime - now) / 3600000);
        return {
          allowed: false,
          reason: `🎬 今日影片生成限額已滿 (${maxDaily}個/天)。請 ${waitHours} 小時後再來。`,
          remaining: 0
        };
      }
      ...
      return {
        allowed: true,
        remaining: Math.min(maxHourly - data.hourly.length, maxDaily - data.daily.length)
      };
    } catch (err) {
      console.error("KV Error:", err);
      return { allowed: true };
...
// =================================================================================
// 影片生成供應商 - Pollinations.ai
// =================================================================================
class PollinationsVideoProvider {
  constructor() {
    this.name = 'pollinations';
    this.baseUrl = CONFIG.VIDEO.providers.pollinations.baseUrl;
    this.models = CONFIG.VIDEO.providers.pollinations.models;
    this.defaultModel = CONFIG.VIDEO.providers.pollinations.defaultModel;
    this.maxDuration = CONFIG.VIDEO.providers.pollinations.maxDuration;
    this.supportedSizes = CONFIG.VIDEO.providers.pollinations.supportedSizes;
  }
  ...
  async generate(prompt, options, logger) {
    ...
    if (!this.models.includes(model)) {
      throw new Error(`不支援的模型: ${model}。可用模型: ${this.models.join(', ')}`);
    }

    if (duration > this.maxDuration) {
      throw new Error(`影片長度不能超過 ${this.maxDuration} 秒`);
    }

    const sizeSupported = this.supportedSizes.some(
      s => s.width === width && s.height === height
    );
    if (!sizeSupported) {
      throw new Error(`不支援的尺寸: ${width}x${height}。可用尺寸: ${this.supportedSizes.map(s => `${s.width}x${s.height}`).join(', ')}`);
    }
    ...
    url.searchParams.append('duration', Math.min(duration, this.maxDuration).toString());
    ...
    logger.log(`🎬 [PollinationsVideo] 生成影片: ${model} ${width}x${height} ${duration}s`);
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(`Pollinations API 錯誤 (${response.status}): ${errorText}`);
      }
      ...
      logger.log(`✅ [PollinationsVideo] 影片生成成功，大小: ${videoBlob.size} bytes`);
      ...
      logger.log(`❌ [PollinationsVideo] 生成失敗: ${error.message}`);
      throw error;
    }
  }
...
class VideoProviderRouter {
  ...
  initializeProviders() {
    // 註冊影片生成供應商
    if (CONFIG.VIDEO.providers.pollinations.enabled) {
      this.providers.set('pollinations', new PollinationsVideoProvider());
    }
  }

  getProvider(providerName = null) {
    const name = providerName || CONFIG.VIDEO.defaultProvider;
    const provider = this.providers.get(name);
    
    if (!provider) {
      throw new Error(`不支援的影片供應商: ${name}。可用供應商: ${Array.from(this.providers.keys()).join(', ')}`);
    }
  ...
  async generate(prompt, options, logger) {
    const providerName = options.provider || CONFIG.VIDEO.defaultProvider;
    const provider = this.getProvider(providerName);
    
    logger.log(`🎬 [VideoRouter] 使用供應商: ${providerName}`);
    
    try {
      const result = await provider.generate(prompt, options, logger);
      return result;
    } catch (error) {
      logger.log(`❌ [VideoRouter] 生成失敗: ${error.message}`);
      
      // 如果有重試次數，嘗試其他供應商
      if (CONFIG.VIDEO.maxRetries > 0 && this.providers.size > 1) {
        const otherProviders = Array.from(this.providers.keys())
          .filter(name => name !== providerName);
        
        for (const otherName of otherProviders) {
          try {
            logger.log(`🔄 [VideoRouter] 嘗試備用供應商: ${otherName}`);
            const otherProvider = this.providers.get(otherName);
            const result = await otherProvider.generate(prompt, options, logger);
            logger.log(`✅ [VideoRouter] 備用供應商成功: ${otherName}`);
            return result;
          } catch (retryError) {
            logger.log(`❌ [VideoRouter] 備用供應商失敗: ${otherName} - ${retryError.message}`);
          }
        }
      }
  ...
  getProviderModels(providerName = null) {
    const provider = this.getProvider(providerName);
    return provider.getModels();
  }

  getProviderSizes(providerName = null) {
    const provider = this.getProvider(providerName);
    return provider.getSupportedSizes();
  }

  getMaxDuration(providerName = null) {
    const provider = this.getProvider(providerName);
    return provider.getMaxDuration();
  }
}
...
export default {
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      if (url.pathname === '/nano') {
      ...
      else if (url.pathname === '/_internal/video/generate') {
        response = await handleVideoGenerate(request, env, ctx);
      }
        ...
        response = new Response(JSON.stringify({ error: 'Not Found', message: '此 Worker 僅提供 Web UI 界面', available_paths: ['/', '/health', '/_internal/generate', '/_internal/video/generate', '/nano'] }), { status: 404, headers: corsHeaders({ 'Content-Type': 'application/json' }) });
      }
...
async function handleVideoGenerate(request, env, ctx) {
  const logger = new Logger();
  ...
  try {
    ...
    // 解析請求
    const body = await request.json();
    const { prompt, model, width, height, duration, seed, negativePrompt, provider } = body;
    ...
    const limitCheck = await rateLimiter.checkVideoLimit(ip);
    ...
    logger.log(`🎬 [VideoGenerate] 收到影片生成請求: ${prompt.substring(0, 50)}...`);
    logger.log(`📊 參數: model=${model}, size=${width}x${height}, duration=${duration}s, provider=${provider}`);

    // 初始化影片供應商路由器
    const videoRouter = new VideoProviderRouter();

    // 構建生成選項
    const options = {
      model: model || CONFIG.VIDEO.providers.pollinations.defaultModel,
      width: width || 512,
      height: height || 512,
      duration: duration || 4,
      seed: seed || null,
    ...
    // 生成影片
    const result = await videoRouter.generate(prompt, options, logger);

    logger.log(`✅ [VideoGenerate] 影片生成成功: ${result.provider}/${result.model}`);
    ...
    logger.log(`❌ [VideoGenerate] 錯誤: ${error.message}`);
